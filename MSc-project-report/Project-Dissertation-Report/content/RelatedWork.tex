...
Repage was introduced in 2006 by Sabater et. al\cite{Sabater2006} and aims to establish two different aspects to trust modelling, Image and Reputation, defining them as distinct social evaluations of a target agent.

Image is defined as the agent's personal belief about a certain property of the target agent, be it a physical, mental or social trait. Reputation is a meta-belief about an impersonal evaluation of the target, in other words, it is the belief on the evaluation being circulated about the target. On a more concrete level, reputation is distinguished between \textit{shared evaluation} and \textit{shared voice}. Considering that an agent has beliefs about how other agents evaluate a certain target, if in a set of agents this beliefs converge we can say that there exists a \textit{shared evaluation} of the target. It's important to note that all sharing agents are known and well defined.
