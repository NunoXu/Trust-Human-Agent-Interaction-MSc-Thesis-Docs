...
\begin{itemize}
	...
	\item $S_R$: represents probabilities, including a set of constants, $C_R$, with a label written as \overline{$r$};
	\item $S_A$: represents agent names;
	\item $S_F$: represents formulas.
\end{itemize}

Predicate symbols are identified by their sorts. For example the predicate symbol B is written B(SA,SF), this means that B is a binary predicate symbol with first argument in the sort SA and the second argument in SF.  The sort SR represents probabilities and includes a sets of constant, CR, with a label r for each rational number r ∈ [0,1]?I
Q. The sort SA represents
agents’ names and includes CA ={i1, . . . , in}, a finite set of agents identifiers . Finally, SF represents formulas and contains the set CF of constant formulas, that is built recursively and simultaneously
with the construction of the language. First, we add the constant ?ϕ? for each ϕ ∈ Fm(Lbasic) and then, given a formulaΨ ∈ Fm(Lag), we also add the constant ?Ψ? toCF. The set of formulas Fm(Lag) of the language is defined using the standard syntax of classical first-
order logic, and it has the special predicates B,Pr≤,Pr≥,S. The intended meaning of these predicate symbols is the following
