...
\begin{itemize}
	...
	\item a pair (\textbf{$\tau$}) composed by the action $\alpha$ entrusted to the trustee, that possibly produces an outcome $p$, $(\alpha, p)$ that is contained in the goal of \textbf{X} ($g_x$);
	\item the goal of the trustor (\textbf{$g_x$}).
\end{itemize}
