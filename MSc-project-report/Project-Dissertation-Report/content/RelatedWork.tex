...
\begin{itemize}
	...
	\item a pair ($\bm{\tau}$) composed by the action $\alpha$ entrusted to the trustee, that possibly produces an outcome $p$, $(\alpha, p)$ that is contained in the goal of \textbf{X} ($g_x$);
