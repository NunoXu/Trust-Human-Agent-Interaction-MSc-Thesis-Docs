...
\begin{itemize}
	...
	\item a pair ($\bm{\tau}$) composed by the action \bm{$\alpha$} entrusted to the trustee, that possibly produces an outcome \bm{$\rho$}, that is contained in the goal of X ($g_x$); represented by $(\alpha, \rho)$
...
The model also conceptualizes \textbf{Expectation} as when agent \textbf{X} waits for \
