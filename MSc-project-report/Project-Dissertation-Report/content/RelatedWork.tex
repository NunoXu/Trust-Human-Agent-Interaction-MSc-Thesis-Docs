...
Where:
\begin{itemize}
	\item $DoC_x[Opp_y(\alpha, \rho)]$ is the \ac{DoC} of all contextual factors in which Y will act; in other words, the degree of Opportunity Y has to do $\alpha$ and result in $\rho$;
	\item $DoC_x[Ability_y(\alpha)]$ is the \ac{DoC} of Y's ability to perform $\alpha$;
	\item $DoC_x[WillDo_y(\alpha)]$ is the \ac{DoC} of Y's ability to perform $\alpha$;
