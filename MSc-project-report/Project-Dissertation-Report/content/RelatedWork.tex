...
As stated in the definition (Section \ref{subsubsec:CastelfranchiTrust}) the mental attitude of the trustor is defined by beliefs of the qualities and faults of Y, so when can quantify the strength of our belief in this attribute by defining it's \textbf{Degree of Credibility} (DoC), which is defined by a function \textbf{F} that takes in the trustor \textbf{X}, trustee \textbf{Y} and task \bm{$\tau$}, as shown in equation \ref{eq:DegreeOfCredibility}. 
\begin{equation}
	\begin {aligned}
