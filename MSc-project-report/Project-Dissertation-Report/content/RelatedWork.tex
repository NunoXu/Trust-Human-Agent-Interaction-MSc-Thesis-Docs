...
The model also conceptualizes \textbf{Expectation} as when agent X waits for $\rho$ to happen when an action $\alpha$ trusted to Y is being performed, formalized in first order logic in equation 

\begin{equation}
	(\text{\textit{Expectation}}\ X\ \rho) \implies 
		(Bel_x^{t'} (\text{\textit{will-be-true}}^{t''} \rho)) \wedge (Goal_x^{Period(t', t''')} (KnowWhether_X (\rho\ OR\ \text{\textit{Not}} \rho)^{t''})
\end{equation}
